...
void write_kmer_distr(NanoporeHDP* nhdp, const char* kmer, double* eval_grid, int64_t grid_length) {
    char* filename;
...
void write_all_kmer_distrs(NanoporeHDP* nhdp, double* eval_grid, int64_t grid_length) {
    char* x_filename;
    sprintf(filename, "%s/x_vals.txt", DISTR_DIR);
    FILE* x_vals = fopen(x_filename);
    for (int64_t i = 0; i < grid_length - 1; i++) {
        fprintf(out, "%.17lg\n", eval_grid[i]);
    }
    fprintf(out, "%.17lg", eval_grid[grid_length - 1]);
    fclose(x_vals);
    ...
    for (int64_t kmer_index = 0; i < num_kmers; i++) {
...
int main(int argc, char** argv) {
    ...
    update_nhdp_from_alignment_with_filter(nhdp, alignment_filepath, false);
